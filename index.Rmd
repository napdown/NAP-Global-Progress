...
fig <- plot_ly(measures, x = ~LDCs, y = ~Measure, type = 'bar', orientation = 'h', mode="text", text=~LDCs, textposition= 'middle left',textfont=list(size=12, color='white'), name = 'LDCs',
...
fig<-fig%>%add_annotations(text = measures$`Developing Countries  (non-LDC)`,x=~`Developing Countries  (non-LDC)`,y=~Measure,
                           font=list(family='Arial', size=10, textposition='middle', align='right',color='white'),showarrow=FALSE)

fig<-fig%>%add_annotations(text = measures$`Without Measure`,x=~`Without Measure`,y=~Measure,
                           font=list(family='Arial', size=10, textposition='middle', align='right',color='white'),showarrow=FALSE)
fig<-fig%>%add_annotations(text = measures$LDCs,x=~LDCs,y=~Measure,
                           font=list(family='Arial', size=10, textposition='bottom', align='right',color='white'),showarrow=FALSE)
...
plot_ly(ldc,type='bar',x=~countryname,y=~Total, color = ~Category2, colors = 'Blue', showlegend=F)%>%
  plotly::layout(barmode='stack', xaxis = list(title = "", tickangle=90, tickfont=list(size=18)),yaxis=list(range=c(0,450), tickfont=list(size=18), title='Total Amount ((Million USD)', titlefont=list(size=18)))
```
...
plot_ly(otherdc,type='bar',x=~countryname,y=~Total, color = ~Category2, colors = 'Blue', showlegend=F)%>%
 plotly::layout(barmode='stack', xaxis = list(title = "", tickangle=90, tickfont=list(size=15.5)),yaxis=list(range=c(0,450), tickfont=list(size=15.5), title='Total Amount ((Million USD)', titlefont=list(size=15.5)))
```
