...
Readiness<- read_excel("Open_NAPs_Database.xlsm",sheet = "Readiness")%>%dplyr::filter(Activity=='Adaptation Planning (AP)')
...
plotC<- mdf%>%filter(Element=="C. Implementation strategies")%>%plot_ly(type='bar',y=~value,x=~Measure1wrap,text=~value, textposition= 'middle',textfont=list(size=12, color='white'), color = ~variable, colors = c('#512679', '#8567a1','#b9a8c9'),bargap=0.1, showlegend=F)%>%
  plotly::layout(barmode='stack', xaxis = list(title = "C. Implementation strategies"),yaxis=list(title='No. of Countries'))

plotD<-mdf%>%filter(Element=="D. Reporting, monitoring and review")%>% plot_ly(type='bar',y=~value,x=~Measure1wrap, text=~value, textposition= 'middle',textfont=list(size=12, color='white'), color = ~variable, colors = c('#512679', '#8567a1','#b9a8c9'),autosize=F,  showlegend=T)%>%
