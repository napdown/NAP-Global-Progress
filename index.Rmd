...
bscols(
  plot_ly(shared_df)%>%
  add_pie(hole=0.6,labels = ~`Delivery Partner`, values = ~`Readiness support approved`, text=sapply(df$`Delivery Partner`, FUN = function(`Delivery Partner`){paste(strwrap(`Delivery Partner`, width = 30), collapse = "<br>")}) , textinfo='labels', rotation=120, showlegend=F) %>%
  layout(title="",
            font=list(size=10))
)
```
