...
output: 
  flexdashboard::flex_dashboard:
    ...
    smooth_scroll: true
    source_code: https://github.com/napdown/NAP-Global-Progre/index.Rmd
    theme: paper
...
```{r}
m<-tracker_merge%>%group_by(Measure,`Measure code`,Country, Category, Element)%>%count(Country)%>%
  summarise('Countries'=sum(n))
...
#m$mytext<-sapply(m$Country, FUN = function(Country){paste(strwrap(Country, width = 20), collapse="<br>")})
...
### By Region
```{r}
reg<-tracker_merge%>%group_by(Measure,`Measure code`,Region, Country, Element)%>%count(Country)%>%
  summarise('Countries'=sum(n))

reg$Measurewrap<-sapply(reg$Measure, FUN = function(Measure){paste(strwrap(Measure, width = 50), collapse = "<br>")})

reg$Measurewrap<-factor(reg$Measurewrap, levels = unique(reg$Measurewrap)[order(reg$`Measure code`,decreasing = F)])

shared_reg<-SharedData$new(reg)
shared_reg%>%
  plot_ly(type = 'bar',
    x=~Countries,
    y=~Region,
...
filter_select("Element", "Select Element", shared_tracker, ~Element, multiple=TRUE, allLevels = TRUE)
...
_To Deselect, delete the item from filter box_
...
3. Then from the drop down menu on the second column select how you want to represent your values e.g as a Count, Sum, List, etc. See screenshot.    
