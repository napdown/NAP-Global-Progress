...
output: 
  flexdashboard::flex_dashboard:
    ...
    smooth_scroll: true
    source_code: https://github.com/napdown/NAP-Global-Progress
    theme:
...
filter_select("Element", "Select Measure", shared_measures, ~Element, multiple=TRUE, allLevels = TRUE)
...
```{r}
m<-tracker_merge%>%group_by(Measure,`Measure code`,Region, Category, Category2, Element)%>%count(Country)%>%
  summarise('Countries'=sum(n))

m$Measurewrap<-sapply(m$Measure, FUN = function(Measure){paste(strwrap(Measure, width = 50), collapse = "<br>")})

m$Measurewrap<-factor(m$Measurewrap, levels = unique(m$Measurewrap)[order(m$`Measure code`,decreasing = F)])

shared_tracker<-SharedData$new(m)

shared_tracker%>%
  plot_ly(type = 'bar',
    x=~Countries,
    y=~Category,
   group = ~Element, color = ~Measure
...
##### gg
```{r}
x<-ggplot(shared_tracker, aes(Countries, `Measure code`))+
...
filter_select("Element", "Select Measure", shared_tracker, ~Element, multiple=TRUE, allLevels = TRUE)
filter_select("Measure", "Select Measure", shared_tracker, ~Measure, multiple=TRUE, allLevels = TRUE)
...
p<-ggplot(country_tot)+
  geom_bar(stat='identity', aes(y=countryname,x=Total), colour='white',fill='blue')+
  labs(x='Amount Accessed (Million USD)', y='Country')+
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=0.5, size = 11), axis.text.y = element_text(size = 11), axis.title.x = element_text(size = 11), axis.title.y = element_blank())
ggplotly(p)
...
plot_ly(ldc,type='bar',y=~countryname,x=~Total, colors = 'Blue', showlegend=F)%>%
  plotly::layout(xaxis = list(title = "Total Amount ((Million USD)",range=c(0,450), tickangle=90, tickfont=list(size=14)),yaxis=list( tickfont=list(size=14), title='', titlefont=list(size=16)))
```
...
plot_ly(otherdc,type='bar',y=~countryname,x=~Total, colors = 'Blue', showlegend=F)%>%
 plotly::layout(xaxis = list(title = "Total Amount ((Million USD)", range=c(0,450),tickangle=90, tickfont=list(size=15.85)),yaxis=list( tickfont=list(size=12), title='', titlefont=list(size=12)))
```
...
GCF Resources - Interactive {style="position:relative;"}
============================================================
...
To deselect, delete the item from filter box


Row {style="height:100pc;"}
------------------------------
### Total GCF by Country {data-commentary-width=400}
```{r}
...
```{r}
popup<-paste(df$countryname,'<br>', 'Total GCF:', df$`Grand Total GCF`,'<br>', 'Total Readiness:', df$`Readiness support approved`)
...
shared_df%>%
  ...
  addCircleMarkers(fill = T, radius = ~`Grand Total GCF`*0.1,weight =~`Readiness support approved`, color = '#fc7204',
    label = lapply(as.list(paste('<strong><strong>',df$countryname,'<br>', 'Total GCF:', df$`Grand Total GCF`,'<br>', 'Total Readiness:', df$`Readiness support approved`)), HTML),popup=paste(df$countryname,'<br>', 'Total GCF:', df$`Grand Total GCF`,'<br>', 'Total Readiness:', df$`Readiness support approved`)
...
```{r}
filter_select("Region", "Filter by Region", ldcf_shared, ~Region, multiple=TRUE, allLevels = TRUE)

filter_checkbox("Country Category", "Filter by Category", ldcf_shared, ~Category2,columns = 2, inline=T, allLevels = FALSE)

filter_select("Cat2", "More Categories", ldcf_shared, ~Category, multiple=TRUE)
 
filter_checkbox("Status","Filter by Status",ldcf_shared,~Status)
```

Column
----------------------------------------------------------
### LDCF {data-commentary-width=400}
```{r}
ldcf_shared%>%
...
2. Drag-&-Drop the variable(s) of interest from the first column into the second column. To remove variables from the active (second column) drag-&-drop them back to the first column      
3. Then from the drop down menu on the second column select how you want to represent your values e.g as a Count, Sum, List, etc    
See screenshot.    
![how to work the pivot table](pivottable_how_to.png){width=5in}


```{r}
rpivotTable(data=tracker, cols =c('Measure code', 'Measure', 'Region'), rows = c('Measure code', 'Measure', 'Region'),  rendererName ='Row Heatmap', vals = 'Measure', aggregatorName = 'Count')
```
