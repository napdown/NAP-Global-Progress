...
output: 
  flexdashboard::flex_dashboard:
    ...
    smooth_scroll: true
    source_code: https://github.com/napdown/NAP-Global-Progress/index.html
    theme: paper
...
column {data-width=200, data-height=350}
-----------------------------------------
...
column {data-width=250, data-height=250}
-----------------------------------------
...
#df$`Delivery Partner`<-sapply(df$`Delivery Partner`, FUN = function(`Delivery Partner`){paste(strwrap(`Delivery Partner`, width = 50), collapse = "<br>")})

#mylabel <- paste(df$`Delivery Partner`,paste('$',prettyNum(format(sum(df$`Readiness support approved`),big.mark = ","), "", sep = ",")),  sep = " \n")

bscols(
  plot_ly(shared_df)%>%
  add_pie(hole=0.6,labels = ~`Delivery Partner`, values = ~`Readiness support approved`, text=sapply(df$`Delivery Partner`, FUN = function(`Delivery Partner`){paste(strwrap(`Delivery Partner`, width = 15), collapse = "<br>")}), textinfo='labels', rotation=130, showlegend=F) %>%
  layout(title="",
            font=list(size=10))
)
```
...
plot_ly(regl)%>%
  add_pie(hole=0.6, labels = ~Region, values = ~Total, text=~Region , textinfo='labels',rotation=118, showlegend=F) %>%
  layout(title="",font=list(size=11))
...
plot_ly(regs)%>%
  add_pie(hole=0.6, labels = mytext, values = ~Total, text=~Region , textinfo=  mytext, textposition='auto', rotation=210, showlegend=F) %>%
  layout(title="",font=list(size=11))
